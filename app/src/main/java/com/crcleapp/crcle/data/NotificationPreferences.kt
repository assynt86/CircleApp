...
class NotificationPreferencesStore(private val context: Context) {
    // Making these public/accessible if needed, but keeping keys private
    companion object {
    ...
    val friendRequestsEnabledFlow: Flow<Boolean> = context.notificationDataStore.data
        .map { preferences -> preferences[FRIEND_REQUESTS_ENABLED_KEY] ?: true }

    val friendRequestAcceptedEnabledFlow: Flow<Boolean> = context.notificationDataStore.data
        .map { preferences -> preferences[FRIEND_REQUEST_ACCEPTED_ENABLED_KEY] ?: true }

    val circleInvitesEnabledFlow: Flow<Boolean> = context.notificationDataStore.data
        .map { preferences -> preferences[CIRCLE_INVITES_ENABLED_KEY] ?: true }

    val newPhotosEnabledFlow: Flow<Boolean> = context.notificationDataStore.data
        .map { preferences -> preferences[NEW_PHOTOS_ENABLED_KEY] ?: true }

    suspend fun setNotificationsEnabled(enabled: Boolean) {
        context.notificationDataStore.edit { it[NOTIFICATIONS_ENABLED_KEY] = enabled }
    }

    suspend fun setFriendRequestsEnabled(enabled: Boolean) {
        context.notificationDataStore.edit { it[FRIEND_REQUESTS_ENABLED_KEY] = enabled }
    }

    suspend fun setFriendRequestAcceptedEnabled(enabled: Boolean) {
        context.notificationDataStore.edit { it[FRIEND_REQUEST_ACCEPTED_ENABLED_KEY] = enabled }
    }

    suspend fun setCircleInvitesEnabled(enabled: Boolean) {
        context.notificationDataStore.edit { it[CIRCLE_INVITES_ENABLED_KEY] = enabled }
    }

    suspend fun setNewPhotosEnabled(enabled: Boolean) {
        context.notificationDataStore.edit { it[NEW_PHOTOS_ENABLED_KEY] = enabled }
    }
}
